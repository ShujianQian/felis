...
class LocalRMWTxn : public Txn<RMWState>, public RMWStruct {
    Client *client;
  public:
    LocalRMWTxn(Client *client, uint64_t serial_id);
...
LocalRMWTxn::LocalRMWTxn(Client *client, uint64_t serial_id)
    : Txn<RMWState>(serial_id),
      RMWStruct(client->GenerateTransactionInput<RMWStruct>()),
      client(client)
{}

void LocalRMWTxn::Prepare()
{
  if (!VHandleSyncService::g_lock_elision) {
    ...
    static constexpr auto LookupIndex = [](auto k, int i, auto state, auto handle) {
      auto &rel = util::Instance<TableManager>().Get<ycsb_dist::YcsbDist>();
      YcsbDist::Key dbk;
      dbk.k = k;
      void *buf = alloca(512);
...
void LocalRMWTxn::WriteRow(TxnRow vhandle)
{
...
void LocalRMWTxn::ReadRow(TxnRow vhandle)
{
  vhandle.Read<YcsbDist::Value>();
}

void LocalRMWTxn::Run()
{
          ...
          if (i == kTotal - Client::g_extra_read) {
            ...
            root->AttachRoutine(
                ...
                [](auto &ctx) {
                  ...
                  TxnRow vhandle = handle(state->rows[i]);
                  auto v = vhandle.Read<YcsbDist::Value>();
...
class DistRMWTxn : public Txn<RMWState>, public RMWStruct {
  Client *client;
public:
  LocalRMWTxn(Client *client, uint64_t serial_id);
  void Run() override final;
...
{
  return new LocalRMWTxn(this, serial_id);
}
