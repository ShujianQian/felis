...
void EpochClient::OnExecuteComplete()
{
  stats.execution_time_ms += callback.perf.duration_ms();
    ...
    PriorityTxnService::init_piece_time += (int) ((PriorityTxnService::g_initialize_end_tsc - PriorityTxnService::g_initialize_start_tsc)) / 2200000;
    PriorityTxnService::insert_piece_time += (int) ((PriorityTxnService::g_insert_end_tsc - PriorityTxnService::g_tsc)) / 2200000;
  if (NodeConfiguration::g_priority_txn) {
    auto &svc = util::Instance<PriorityTxnService>();
    for (int i = 0; i < NodeConfiguration::g_nr_threads; ++i) {
      auto pcs = svc.PriPcCnt[i]->Get();
